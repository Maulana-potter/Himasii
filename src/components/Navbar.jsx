...
const Navbar = () => {
  ...
  return (
    ...
    >
      <div className="container mx-auto px-6 flex justify-between items-center">
        ...
        <NavLink to="/" className="flex items-center gap-3 group">
          ...
          <div className="flex flex-col text-left">
            ...
            <span className="text-[10px] font-bold tracking-[0.3em] uppercase text-primary text-left">
              UMSU
            </span>
          </div>
        </NavLink>
        ...
        <div className="hidden md:flex items-center gap-8">
          {[
            ...
            <NavLink
              key={item.name}
              to={item.path}
              // End digunakan agar rute Home tidak selalu aktif
              end={item.path === "/"}
              className={({ isActive }) => `
                font-bold text-sm uppercase tracking-widest transition-colors duration-300 relative group
      ...
      {/* === MOBILE DROPDOWN === */}
      <div
        className={`md:hidden absolute top-full left-0 w-full bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-t border-border shadow-2xl transition-all duration-500 overflow-hidden ${
        ...
        <div className="flex flex-col p-6 space-y-4 text-left">
          {[
            ...
            <NavLink
              ...
              className={({ isActive }) => `
                text-lg font-bold pl-4 border-l-4 transition-all text-left
                ${isActive ? "border-primary text-primary bg-primary/5 py-2 rounded-r-lg" : "border-transparent text-slate-700 dark:text-white hover:text-primary"}
              `}
            >
              {item.name}
            </NavLink>
